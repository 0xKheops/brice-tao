...
2. **Ranked profitable subnets** — an ordered list of subnet netuids scored by momentum (best first), already filtered for liquidity and quality gates (volume, market cap, holder count, pool liquidity, registration status, emission). Immune subnets bypass all gates except the score gate. The score gate uses hysteresis: held subnets only need `minScore − INCUMBENCY_BONUS` (65) to stay in the candidate pool, while new subnets need `minScore` (70) to enter.
