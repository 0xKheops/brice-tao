 ...
 * ## Algorithm
 *
 * 1. Fetch the subnet metagraph (hotkeys, validator permits, alpha stakes,
 *    and alpha dividends per hotkey).
 * 2. Filter to validators with a valid permit AND non-zero alpha stake.
 * 3. Compute `yieldPerAlpha = alphaDividends / alphaStake` for each candidate.
 * 4. Sort by yield descending, then by stake descending, then by UID ascending
 *    as tiebreakers.
 *
 * ## Limitations
 *
 * - Only uses last-epoch data (no historical trend)
 * - Ignores take rate / commission
 * - Ignores validator uptime and reliability
 * - No blacklist/whitelist support
 *
